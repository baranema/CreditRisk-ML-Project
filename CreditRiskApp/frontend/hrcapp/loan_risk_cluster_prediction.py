...
API_URL = 'https://backend-service-kx4s6cgoga-lm.a.run.app/application_clustering/'
...
def risk_sense():
    # Streamlit app
    st.title('RiskSense: Application clustering algorithm based on their credit risk')

    og_df = pd.read_csv(
        'default_data/sampled_application.csv',
        dtype={'SK_ID_CURR': int},
    )

    # Add a file uploader button
    uploaded_file = st.file_uploader(
        'Upload a CSV file or use the default sampled_application.csv', type='csv',
    )
    ...
    try:
        if session_state.first_run:
            ...
            if status == 200:
                df['cluster'] = predictions[0]['clusters']
        ...
        df.insert(2, 'cluster', df.pop('cluster'))
        ...
        categorical_column = st.selectbox(
            'Select:', ['Nothing Selected'] + ['cluster'] +
